...
<section class="page-section ready-dispatch-page">
    <div class="page-header">
        <div>
            <h1>Ready Dispatch</h1>
            <p>Orders fully shipped by sellers and waiting for delivery agent assignment.</p>
        </div>
    </div>
    ...
    <div class="approval-card">
        <div class="approval-header">
            <div>
                ...
                <p class="account-subtitle">Only orders with every item marked shipped and no delivery assignment appear here.</p>
            </div>

            <div class="approval-tools order-tools">
                <div class="search-box-small">
                    <i class="fa-solid fa-magnifying-glass"></i>
                    <input type="text" id="readyDispatchSearch" placeholder="Search queue...">
                </div>
            </div>
        </div>

        <table class="approval-table order-table ready-dispatch-table">
            <thead>
                <tr>
                    <th>Order</th>
                    <th>Customer</th>
            ...
            <tbody>
                <?php if (empty($orders)): ?>
                    <tr>
                        <td colspan="9" class="empty-cell">No orders are ready for dispatch.</td>
                    </tr>
                <?php endif; ?>
                ...
                <?php foreach ($orders as $order): ?>
                    ...
                    ?>
                    <tr data-ready-dispatch-row data-search="<?= htmlspecialchars($searchText) ?>">
                        <td>
                        ...
                        <td>
                            <span class="approval-status dispatch-status-ready">
                                Ready
                            </span>
                        </td>
        ...
        <div class="table-footer">
            <p id="readyDispatchCountText">Showing <?= count($orders) ?> order<?= count($orders) === 1 ? '' : 's' ?></p>
        </div>
    </div>
</section>
